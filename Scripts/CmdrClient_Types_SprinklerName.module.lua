...
return function(p1) --[[ Line: 19 ]]
    ...
    local u2 = {};
    for _, v3 in require(l__ReplicatedStorage__1:WaitForChild("SharedModules"):WaitForChild("SprinklerData")) do
        table.insert(u2, v3.SprinklerName);
    end;
    ...
    p1:RegisterType("sprinklerName", {
        Validate = function(p9) --[[ Name: Validate, Line 29 ]]
            -- upvalues: u8 (copy), u2 (copy)
            return u8(p9, true) ~= nil, string.format("%q is not a valid sprinkler. Valid: %s", p9, table.concat(u2, ", "));
        end,
