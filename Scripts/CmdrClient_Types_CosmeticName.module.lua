...
return function(p1) --[[ Line: 19 ]]
    ...
    local u2 = {};
    for _, v3 in require(l__ReplicatedStorage__1:WaitForChild("SharedModules"):WaitForChild("PropData")).Data do
        table.insert(u2, v3.PropName);
    end;
    ...
    p1:RegisterType("cosmeticName", {
        Validate = function(p9) --[[ Name: Validate, Line 29 ]]
            -- upvalues: u8 (copy), u2 (copy)
            return u8(p9, true) ~= nil, string.format("%q is not a valid cosmetic. Valid cosmetics: %s", p9, table.concat(u2, ", "));
        end,
